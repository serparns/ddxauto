...
export default class  ClubsRequests extends BaseRequests {
    ...
    async getClubById(status: number, parameters: object, id: number): Promise<APIResponse> {
        return await this.get(`${this.baseUrl}${paths.paths.clubs}/${id}`, status, parameters);
    }
}
