...
test.describe("Api-тест на создание юзера с клуюом и получения данных о нем", async () => {
    test("[positive] получить юзера с подстановкой idклуба из запроса", async ({request}) => {
        const requestsClubs = await new ClubsRequests(request).getClubById(200, await getBaseParameters());
        const clubID = (await requestsClubs.json()).data[0].id;
        const requestBody = {
            session_id: "123",
            request_id: "321",
            request_source: "crm",
        ...
        const response = await new UserRequests(request).getUser(200, await getBaseParameters(),(await createUser.json()).data.id,
            (await createUser.json()).data.phone, (await createUser.json()).data.email);
