...
async function globalSetup() {
    ...
    process.env['GROUP_TRAINING_DATA'] = JSON.stringify((await (await new GroupTrainingRequests(request).getGroupTraining(Statuses.OK, await getBaseParameters())).json()).data[0])
}
